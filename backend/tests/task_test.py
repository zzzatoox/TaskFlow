...
async def test_get_all_tasks(client: AsyncClient):
    ...
    task_response = await client.post("/tasks", headers=headers, json=TEST_TASK_DATA)
...
async def test_get_task_by_id(client: AsyncClient):
    ...
    login_response = await client.post(
        "/token",
        data={
            "username": TEST_USER_DATA2["login"],
            "password": TEST_USER_DATA["password"],
        },
    )
    ...
    task_create_response = await client.post(
        "/tasks", headers=headers, json=TEST_TASK_DATA
    )
    assert task_create_response.status_code == 200
    task_data = task_create_response.json()

    task_response = await client.get(f"/tasks/{task_data['id']}", headers=headers2)
    assert task_response.status_code == 403
    task_response = await client.get(f"/tasks/{task_data['id']}", headers=headers)
    assert task_response.status_code == 200
...
async def test_update_task(client: AsyncClient):
    ...
    task_update_response = await client.put(
        f"/tasks/{task_data['id']}", headers=headers, json=task_data
    )
...
async def test_delete_task(client: AsyncClient):
    ...
    task_delete_response = await client.delete(
        f"tasks/{task_data['id']}", headers=headers
    )
